...
let config = {
  ...
  statecolours: {
    ...
    food: { 
      0: "black", 
      1: "#E6F3FF", 2: "#CCE6FF", 3: "#B3D9FF", 4: "#99CCFF",
      5: "#80BFFF", 6: "#66B2FF", 7: "#4DA6FF", 8: "#3399FF",
      9: "#1A8CFF", 10: "#0080FF", 11: "#0066CC", 12: "#004C99",
      13: "#003366", 14: "#001933", 15: "#00008C" 
    },
    enzyme1: { 
      0: "black", 
      1: "#FFFF00",  // yellow
...
for (let x = 0; x < sim.particles.nc; x++) {
  for (let y = 0; y < sim.particles.nr; y++) {
      sim.particles.grid[x][y] = { food: { bitstring: "", value: 0 } };
  }
}
...
if (state === 1 && this.rng.genrand_real1() < (EnzymeProduction / 100)) {
  ...
  if (
    ...
    RN.species == 1 &&
    sim.particles.grid[i][j].food.value > 0.5 && // Changed from == 1
...
sim.particles.nextState = function (i, j) {
  ...
  for (const foodItem of this.grid[i][j].food) {
      ...
      if (sim.enzyme1.grid[i][j].enzyme1 > 0) {
          ...
          if (index !== -1) {
              // Split the food item into two parts
              const left = foodItem.bitstring.substring(0, index + pattern.length);
              const right = foodItem.bitstring.substring(index + pattern.length);
              
              if (left && right) {
                  // Add both parts to newFoods array
                  newFoods.push({
                      bitstring: left,
                      value: parseInt(left, 2) / (1 << left.length)
                  });
                  newFoods.push({
                      bitstring: right,
  ...
  // Calculate display value (sum of all food values in cell)
  let displayValue = 0;
  this.grid[i][j].food.forEach(food => {
      const ones = (food.bitstring.match(/1/g) || []).length;
      const total = food.bitstring.length;
      displayValue += ones / total;
  });
  
  // Normalize for color mapping (0-15)
  this.grid[i][j].displayValue = Math.min(15, Math.floor(displayValue * 15));
};
...
sim.particles.update = function () {
  ...
  if (Seperator == 1) {
    ...
    const spawnBitstringFood = (iStart, iEnd, jStart, jEnd, chance, length) => {
      for (let i = iStart; i < iEnd; i++) {
        for (let j = jStart; j < jEnd; j++) {
          if (this.grid[i][j].food.bitstring === "" && 
              this.rng.genrand_real1() < chance) {
            
            // Generate random bitstring
            let bitstring = "";
            for (let k = 0; k < length; k++) {
              bitstring += this.rng.genrand_real1() < 0.5 ? "0" : "1";
            }
            
            this.grid[i][j].food = {
              bitstring: bitstring,
              value: parseInt(bitstring, 2) / (1 << length) // Normalized value
            };
          }
        }
      }
    ...
    spawnBitstringFood(0, midCol, 0, midRow, FoodLeft, 8); // 8-bit strings
