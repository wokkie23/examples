  ...
  let config = {
    ...
    statecolours: {
      ...
      displayValue: {  
        ...
        4: "#FF4500",     // 4: Orange Red (length 4)
  ...
  if (state === 1 && this.rng.genrand_real1() < (EnzymeProduction / 100)) {
    ...
    if (
      ...
      RN.species == 1 &&
      hasLengthOneFood(sim.particles.grid[i][j].food) && // Check if any food exists
    ...
    if(foodIndex > -1) {
        ...
        sim.particles.grid[i][j].food = foods;
    }
  ...
  sim.particles.nextState = function (i, j) {
    ...
    for (const foodItem of this.grid[i][j].food) {
        ...
        if (!processed && sim.enzyme2.grid[i][j].enzyme2 > 0) {
          const pattern = "10";
          const index = foodItem.bitstring.indexOf(pattern);
          
          if (index !== -1) {
              // Split into "10" and remainder
              const left = foodItem.bitstring.substring(0, index + pattern.length);
              const right = foodItem.bitstring.substring(index + pattern.length);
              
              if (left && right) {
                  newFoods.push({ bitstring: left, value: parseInt(left, 2)/(1<<left.length) });
                  newFoods.push({ bitstring: right, value: parseInt(right, 2)/(1<<right.length) });
                  processed = true;
                  didSplit = true;
              }
          }
  ...
  sim.particles.update = function () {
    ...
    if (Seperator == 1) {
      ...
      const spawnBitstringFood = (iStart, iEnd, jStart, jEnd, chance, patterns) => {
        for (let i = iStart; i < iEnd; i++) {
          for (let j = jStart; j < jEnd; j++) {
            if (this.rng.genrand_real1() < chance && patterns.length > 0) {
